  ...
  function (state: ThreadsState = initialState, action: Action): ThreadsState {
    switch (action.type) {
      ...
      case ThreadsActions.ADD_THREAD: {
        const thread: Thread = (<ThreadsActions.AddThreadAction>action).payload;
      ...
      case ThreadsActions.ADD_MESSAGE: {
        const thread: Thread = (<ThreadsActions.AddMessageAction>action).payload.thread;
        const message: Message = (<ThreadsActions.AddMessageAction>action).payload.message;
      ...
      case ThreadsActions.SELECT_THREAD: {
        const thread = (<ThreadsActions.SelectThreadAction>action).payload;
        const oldThread = state.entities[thread.id];
